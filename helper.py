...
def medal_tally(df):
    medal_telly = df.drop_duplicates(subset= ['Team', 'NOC','Games', 'Year','Season','City', 'Sport', 'Event', 'Medal'])
...
def data_over_time(df, column):
    nations_over_time = df.groupby('Year')[column].nunique().reset_index()
    nations_over_time.rename(columns = {'Year': 'Edition', column: f'No of {column}'}, inplace = True)
...
def most_successful(df, sport):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(15).merge(df, how='left', on='Name')[
        ['Name', 'count', 'Sport', 'region']].drop_duplicates('Name')
    x.rename(columns={'count': 'Total Medals Won'}, inplace=True)
    return x
...
def most_successful_countrywise(df, country):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(10).merge(df, how='left', on='Name')[
        ['Name', 'count', 'Sport', 'region']].drop_duplicates('Name')
    x.rename(columns={'count': 'Total Medals Won'}, inplace=True)
    return x
